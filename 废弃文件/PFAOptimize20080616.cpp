...
void CPFAOptimize::Example()
{
	...
	OptData.SetFluxSummation( 0.248 );
  ...
  */
	////////////////
	Temp = 0.5;	
	OptData.SetProperty( 1, 0, Temp );
	OptData.SetProperty( 3, 0, Temp );
	OptData.SetProperty( 10, 0, Temp );
	OptData.SetProperty( 12, 0, Temp );
	Temp = 0.25;	
	OptData.SetProperty( 2, 0, Temp );	
	OptData.SetProperty( 4, 0, Temp );	
	OptData.SetProperty( 5, 0, Temp );	
	OptData.SetProperty( 6, 0, Temp );	
	OptData.SetProperty( 7, 0, Temp );	
	OptData.SetProperty( 8, 0, Temp );	
	OptData.SetProperty( 9, 0, Temp );		
	OptData.SetProperty( 11, 0, Temp );	
	

	if( !DoOptimitation( OptData ) )
	{
		return ;
	}
	// 计算其他要素
...
bool CPFAOptimize::DoOptimitation( PFAOptimitationData& OptData )
{
	...
	{	
		...
		lTimes = 0;
		CalDummyFlux( 0.001, OptData );
		while ( true )
		{
			lCount++;
			iFlagCircleOver = 0;
			PipeWaterHammerMap.clear();
			for ( i = 1; i <= OptData.GetPropertyCount( 101 ); i++ )         // 遍历所有环
			{
				dWaterHammer = 0.0;                    
				dTempCloseSubtract = 0.0;
				for ( j = 0; j < OptData.GetPropertyCount( i, 101 ); j++ )     // 遍历环中的所有管道
				{
					double dTempOne = 0.0;
					OptData.GetProperty( i, 101, j, iKeyOfPipe );
					...
					if( 0.3 < dPipeDiameter )          // 应该是阻力计算公式，但没查到，按管道直径来选择公式
					{
						...
						PipeWaterHammerMap.insert( map< int, double >::value_type( iKeyOfPipe, dTemp ) );
				...
				dTemp = -1.0 * OptData.GetPumpEfficiency() * dCloseSubtract / dTempCloseSubtract;
				CirReviseValueMap.insert( map< int, double >::value_type( i, dTemp ) ); // 环的校正流量 = -泵站效率 * 环水头损失 / S
			}
			...
			// 修正管道流量
			iKeyOfPipe = 1;
			for ( i = 1; i <= OptData.GetPropertyCount( 102 ); i++ )     // 遍历管道
			{
				int iKeyOfCir = 0;
				int iKeyOfNextCir = 0;
				long lTime = 0;
				if( false == OptData.GetProperty( iKeyOfPipe, 102, 0, iKeyOfCir ) )
				{
					lTime++;
					iKeyOfCir++;
					if( 5000 < lTime )
				...
				OptData.GetProperty( iKeyOfPipe, 1, dPipeFactFlux );
				dTemp = CirReviseValueMap.find( abs( iKeyOfCir ) )->second;
				if( false == OptData.GetProperty( iKeyOfPipe, 102, 1, iKeyOfNextCir ) ) // 没有对应的管道，即此环不是公共管道，只属于一个环所有
				{
					dPipeFactFlux +=  dTemp;
				}
				else                                         // 管道是公共管道，修正时应加上相临的环的修正值，
				{
					dTempCloseSubtract = CirReviseValueMap.find( abs( iKeyOfNextCir ) )->second;
					dPipeFactFlux = dPipeFactFlux + ( iKeyOfCir / abs( iKeyOfCir ) ) * dTemp
						+ ( iKeyOfNextCir / abs( iKeyOfNextCir ) ) * dTempCloseSubtract;
				}
		...
		// 判断前后2次计算的管径相差是否小于45，如果大于，则KLM = 9
		map< int, double >::iterator IntDouMap;
		for ( i = 1; i <= OptData.GetPropertyCount( 101 ); i++ )           // 遍历环
		{	
			for ( j = 0; j < OptData.GetPropertyCount( i, 101 ); j++ )   // 遍历环中管道
			{
				OptData.GetProperty( i, 101, j, iKeyOfPipe );
				IntDouMap = PipeTempDiameterMap.find( abs( iKeyOfPipe ) );
				double dTempDiameter = IntDouMap->second;
				OptData.GetProperty( iKeyOfPipe, 2, dPipeDiameter );
	...
	{
		int iKeyOfCir = IntVecMapIt;
		vector< int >::iterator IntVecIt = IntVecMapIt->second.begin();
		for ( ; IntVecMapIt->second.endl() != IntVecIt; IntVecIt++ )    // 遍历管道
		{
			KKD( LG( I, J ) )=KD( I, J );      // LG是环-管段矩阵，KDD是管径
			VD( I, J )=1274*fabs( Q( I, J ) )/( pow( KD( I, J ), 2 ) );    // 流速 = 1274 * 管道流量 / 直径平方
		}
	}
	...
	for ( I = 0; I < 34; I++ )         // 遍历所有管道
	{
		C=C+CC( KKD( I )/50 )*LL( I );  // 管道总建设造价
	}
	C1=C*( P*0.01+1.0/TT )+0.86*GA*EE*( 43*1018.52 )/ATA;  // 管网的年折算费用
	...
	while( false == bFlagDummyCloseSubtract )
	{
		...
		for ( i = 1; i <= OptData.GetPropertyCount( 100 ); i++ )  // 遍历环
		{
			...
			for ( j = 0; j < OptData.GetPropertyCount( i, 100 ); j++ )  // 遍历环中所有管道
			{
				...
				dTemp = ( iKeyOfPipe / abs( iKeyOfPipe ) ) * pow( 1.0 / fabs( dPipeDummyFlux ), dM / ( dAlfa + dM ) );
				OptData.GetProperty( iKeyOfPipe, 1, dPipeFactFlux );
				OptData.GetProperty( iKeyOfPipe, 2, dPipeLength );
				dTemp = pow( fabs( dPipeFactFlux ), 2 * dN / ( dAlfa + dM ) ) * dPipeLength * dTemp;// 虚流量水头损失
				PipeDummyWaterHammerMap.insert( map< int, double >::value_type( i, dTemp ) );     
			...
			CirReviseValueMap.insert( map< int, double >::value_type( i, dTemp ) );   // 环I校正流量，0.15不知道	
		}
...
{
	if( 0.1 > dCalDiameter )         // 0-100取100
	{
		return 0.1;
	}
	else 
	{
		if( 0.2 > dCalDiameter )    // 100-200取
		{
			return 50.0 * int( ( dCalDiameter + 0.025 ) / 0.05 ) / 1000.0;
		}
		else 
		{
			if( 1 > dCalDiameter )   // 200-1000 
			{
				return 100.0 * int( ( dCalDiameter + 0.025 ) / 0.1 ) / 1000.0;
			}
			else                        // 大于1000
			{
				return 200.0 * int( ( dCalDiameter - 1.0 ) / 0.2 ) / 1000.0 + 1;
			}
		}
	}
}
