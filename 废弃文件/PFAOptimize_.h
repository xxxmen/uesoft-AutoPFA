...
public:
	...
	bool CalFactFlux( ComponentManager& ComManager, const double& dPrecision, const long& lMaxIterative, PFAOptimitationData& OptData );
...
private:
	...
	// 虚流量平差
	bool CalEconomyDiameterAndWaterHarmer( ComponentManager& ComManager, const double& dPrecision, const long& lMaxIterative, PFAOptimitationData& OptData );
	
	// 计算管道-环矩阵，为各环连续校正法做准备
	bool InitializePipeCircle( PFAOptimitationData& OptData );

	// 根据经济管径寻找标准管径
	bool FindStandardDiameter( PFAOptimitationData& OptData );
	...
	bool DoOptimitation( PFAOptimitationData& OptData );
