                            ...
                            int nColumn  /* = -1 */)
{
    ...
    TRY
    {
        ...
        {
            m_arColWidths.InsertAt((INT_PTR)nColumn, (UINT)0 , (INT_PTR)1 );
            if (!GetVirtualMode())
            {
                for (int row = 0; row < m_nRows; row++) 
                {
...
int CALLBACK CGridCtrl::pfnCellFloatCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	UNUSED_ALWAYS(lParamSort);
	...
	double fValue1 = _tcstod(pCell1->GetText(), NULL );
	double fValue2 = _tcstod(pCell2->GetText(), NULL );
