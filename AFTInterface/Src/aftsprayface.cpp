...
ErrorStatus::FileError AFTSprayFace::Read(JunctionData *pData,Junperisist *pModel)
{
	...
	//6.0下管道流向多出K系数
	pSprayModel->PipeInfo(pSprayData->PipeInfo());
	pSprayModel->DistanceUnit(pSprayData->DistanceUnit());
...
ErrorStatus::FileError AFTSprayFace::Save(JunctionData *pData,Junperisist *pModel)
{
	...
	//6.0下管道流向多出K系数
	pSprayData->PipeInfo(pSprayModel->PipeInfo());
	pSprayData->DistanceUnit(pSprayModel->DistanceUnit());
	AFTTransDataFace face;
